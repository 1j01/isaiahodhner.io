...
const Spanvas = (word, data)=> {
	...
	spanvas.render = ()=> {
		...
		// const original_width = original_width || rect.width; // what was this about? was it supposed to keep this state between renders?
		...
		if (strokes) {
			...
			canvas.width = (((max_x - min_x) * scale) + (padding * 2)) * window.devicePixelRatio;
			canvas.height = Math.max(rect.height, ((max_y * scale) - y_offset) + (padding * 2)) * window.devicePixelRatio;
			...
			const canvas_text_width = canvas.width / window.devicePixelRatio - (padding * 2);
