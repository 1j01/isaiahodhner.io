...
// also NOTE: conditional added so this doesn't get executed on the server
...
save_button.addEventListener("click", (e)=> {
	const datas = all_spanvases.map((spanvas)=>
...
const Spanvas = (word, data)=> {
	...
	setTimeout(()=> {
		...
		const json = localStorage[`multi-medium:${all_spanvases.indexOf(spanvas)}:strokes`];
		if (json) {
...
MultiMedium.Input = ()=> {
	...
	requestAnimationFrame(()=> {
		for (let spanvas of all_spanvases) {
			if (!spanvas.hasData()) {
				spanvas.select();
				break;
			}
...
MultiMedium.setData = (datas)=> {
	datas.forEach((data, i)=> {
		if (data) {
			all_spanvases[i].setData({strokes: deserialize_strokes(data)});
		}
	});
};
