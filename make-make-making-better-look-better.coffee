...
# word_ctxs = []
# console.log "clear word_ctxs before setData which should be the final set of drawStrokes calls"
...
word_ctxs = []
# console.log "clear word_ctxs after setData which should be just before the final set of drawStrokes calls, in a setTimeout used to get computed styles"
...
class Bird
	...
	step: ->
		...
		if @land_at_target
			if Math.random() < 0.05
				@target_x = @x + ((Math.random() * 2) - 1) * 5
				@target_y = @y - 5 - Math.random * 20
		else
			if Math.random() < 0.05
				@target_x = Math.random() * canvas.width
				@target_y = Math.random() * canvas.height
		
		if Math.abs(@vx) + Math.abs(@vy) > 1
			# @flappingness = 0.2
			@doAFlap = true
			if @flapCyclePosition >= Math.PI * 2
				@flapCyclePosition = 0
		else
			# @flappingness *= 0.9
			if @flapCyclePosition >= Math.PI * 2
				@doAFlap = false
		# @flapCyclePosition += @flappingness
		if @doAFlap
			@flapCyclePosition += Math.PI * 2 / 100
	draw: (ctx)->
		ctx.beginPath()
		# ctx.moveTo(@x, @y)
		# ctx.lineTo(@x + 5, @y + 5)
		...
		ctx.moveTo(0, 0)
		ctx.save()
		ctx.rotate(Math.sin(@flapCyclePosition))
		ctx.lineTo(10, 0)
		ctx.restore()
		ctx.moveTo(0, 0)
		ctx.rotate(-Math.sin(@flapCyclePosition))
...
birds = (new Bird(-50, 400) for [0...5])


animate = ->
	width = container.clientWidth
	height = container.clientHeight # integer, right?
	...
	bird.draw(ctx) for bird in birds
